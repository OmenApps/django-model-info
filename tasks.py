...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-model-info runtests.py example_project")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-model-info.rst")
...
def lint(c):
    ...
    """
    c.run("flake8 django-model-info example_project")
...
def release(c, bumpsize=""):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_model_info.__version__))
    c.run("git push --tags")
