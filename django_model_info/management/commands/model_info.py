...
from ._method_attr_utils import (
...
class ModelProcessor:
    ...
    def __init__(self, model, verbosity_option):
        self.model = model
    ...
    def build_model_info(self):
        ...
        new_model.docstring.value = get_model_docstring(self.model)
    ...
    def build_other_field_info(self):
        ...
        for field in field_list:
            if getattr(field, "related_model") is None:
...
class Command(BaseCommand):
    ...
    def get_options(self, options) -> tuple:
        ...
        return verbosity, filter_option, export_option

    def handle(self, *_, **options):
        """Handle the command."""
        self.verbosity, self.filter_option, self.export_option = self.get_options(options)
        ...
        for model in model_list:
            ...
            if self.verbosity > 0:
                processor = ModelProcessor(model, self.verbosity)
    ...
    def filter_models(self):
        ...
        abstract_models = self.append_abstract_models(model_list)
        if abstract_models:
